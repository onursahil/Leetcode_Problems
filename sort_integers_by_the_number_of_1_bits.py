...
def sortByBits(arr):
    bin_count = []
    for num in arr:
        bin_num = "{0:b}".format(int(num))
        bin_count.append(tuple((int(num), bin_num.count("1"))))

    new_list = [ seq[0] for seq in sorted(bin_count, key=lambda element: (element[1], element[0])) ]
