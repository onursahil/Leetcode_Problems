...
where <col1> represents the column c1, <row1> represents the row r1, <col2> represents 
...
def cellsInRange(s):
    ...
    start_index, end_index = alphabet.index(s[0].lower()), alphabet.index(s[-2].lower())
    answer = []
    for char in alphabet[start_index:end_index + 1]:
        for i in range(start_num, end_num + 1):
            element = ""
            element += char.upper() + str(i)
